...
function CompanyDashboard() {
  ...
  const payNow = () => {
    ...
    const payment = {
      ...
      notify_url: "https://student-internship-system.vercel.app/payment-notify",
      order_id,
      items: form.title,
      amount,
      currency,
  ...
  const saveJob = async () => {
    ...
    const companyName = user?.username || user?.companyName || "Unknown Company";
    try {
      if (editId) {
        await axios.put(
          `https://student-internship-system.vercel.app/api/jobs/${editId}`,
          form,
          {
            headers: getAuthHeaders(),
          }
  ...
  const totalVacancies = jobs.reduce(
    (sum, j) => sum + Number(j.vacancy || 0),
    0
  );
  const avgApplicants =
    jobs.length > 0 ? (totalApplicants / jobs.length).toFixed(1) : 0;
  ...
  return (
    <div className={darkMode ? "dashboard dark" : "dashboard"}>
      {/* ----- HIDDEN CHECKBOX FOR SIDEBAR TOGGLE (mobile) ----- */}
      <input type="checkbox" id="sidebar-toggle-checkbox" />
      <label htmlFor="sidebar-toggle-checkbox" className="sidebar-toggle">
        <Menu size={24} />
      </label>
      ...
      <main className="main-content">
        ...
        <div className="topbar">
          <div>
            ...
            <p>Manage internships and track company performance.</p>
          </div>
        ...
        <div className="hero-banner">
          <div className="hero-content">
            ...
            <h1>Build Amazing Internship Programs</h1>
            <p>Manage internship opportunities with integrated payment system.</p>
          </div>
            ...
            {!editId && PAYMENT_ENABLED && (
              <div
                style={{
                  ...
                  padding: "12px",
                  borderRadius: "14px",
                  background: paymentCompleted ? "#dcfce7" : "#eff6ff",
                  color: paymentCompleted ? "#166534" : "#1d4ed8",
                  fontWeight: "700",
                }}
              >
                {paymentCompleted
          ...
          <div className="jobs-grid">
            {filteredJobs.length === 0 ? (
              <div className="empty-state">
                ...
                <p>Create your first internship</p>
              </div>
            ) : (
              filteredJobs.map((job) => (
                <div className="job-card" key={job._id}>
                  ...
                  <div className="tags-row">
                    ...
                    <div className="vacancy-tag">👥 {job.vacancy} Seats</div>
                  </div>
                  <div className="tags-row">
      ...
      {selectedJob && (
        <div className="modal-overlay">
          <div className="job-modal">
            ...
            <span className="modal-category">{selectedJob.category}</span>
            <h1>{selectedJob.title}</h1>
            <p>{selectedJob.description}</p>
            <div className="modal-info">
              <div>💰 {selectedJob.salary}</div>
              <div>👥 {selectedJob.vacancy} Seats</div>
              <div>👨‍🎓 {selectedJob.applicants?.length || 0} Applicants</div>
            </div>
          </div>
        </div>
      )}
