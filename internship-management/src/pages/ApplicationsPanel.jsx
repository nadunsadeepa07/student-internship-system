...
const ApplicationsPanel = ({ companyId, getAuthHeaders, onBack }) => {
  ...
  const fetchApplications = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/company/applications?companyId=${companyId}`,
  ...
  const acceptApplication = async (id, date, time) => {
    try {
      await axios.put(`http://localhost:5000/api/company/application/accept/${id}`, { date, time });
      fetchApplications();
  ...
  const rejectApplication = async (id) => {
    try {
      await axios.put(`http://localhost:5000/api/company/application/reject/${id}`);
      fetchApplications();
